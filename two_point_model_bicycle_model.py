...
class bicycle_model:
    ...
    def __init__(self, x, y, psi, vel, deltaf):
        
        self.x = x
        self.y = y
        self.psi = psi #heading angle
        self.deltaf = deltaf #Initial steering angle
        
        
        self.lr = 2.0
        self.lf = 1.5
        
        self.vel = vel #vehicle speed
        
    def update(self):
        
        self.beta = np.arctan((self.lr / self.lr + self.lf) * np.tan(self.deltaf)) #Get the velocity angle
        
        self.psi_dot = self.vel /self.lr * np.sin(self.beta) #Change in angle

        self.y_dot = self.vel * np.sin(self.psi + self.beta)

        self.x_dot = self.vel * np.cos(self.psi + self.beta)
...
class vehicle:
    ...
    def __init__(self, pos, heading, speed, desired_trajectory, sight_point_distances, Kni, Knp, Kf, sigma):
        ...
        self.speed = speed     
        self.deltaf = 0 #initial steering angle   
...
#        print(sight_point_angles_dot)
        ...
        self.deltaf_dot = (self.Kni * self.sight_point_angles[0] * dt +
                            self.Knp * self.sight_point_angles_dot[0] + 
                            self.Kf * self.sight_point_angles_dot[1] ) 
        
        
        self.detlaf = self.deltaf + self.deltaf_dot * dt
        
        print(self.deltaf_dot * dt)
        ...
        self.heading_dot = self.speed /self.lr * np.sin(self.beta) #Change in angle

        self.heading = self.heading_dot * dt
        
        
        self.y_dot = self.speed * np.sin(self.heading + self.beta)
        self.x_dot = self.speed * np.cos(self.heading + self.beta)
        
        
        self.x = self.pos[0] + self.x_dot * dt
        self.y = self.pos[1] + self.y_dot * dt
...
if __name__ == '__main__':
    ...
    myCar = vehicle(pos = center_curve[0] + np.array([0, 0]), heading = 0, speed = speed, 
                    ...
                    Kni = 0, Knp = 0, Kf = -1, 
                    sigma = 0.05) #initalise a car object
    
    
    plt.arrow(myCar.pos[0], myCar.pos[1], 
                      myCar.forward_vector[0] * 20 , myCar.forward_vector[1] * 20,
                               head_width=5, head_length=5, color = 'k')
